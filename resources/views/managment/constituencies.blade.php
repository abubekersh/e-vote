<x-dashboard>
    ...
    <section class="py-10">
        <form action="{{route('constituency')}}" method="post" enctype="multipart/form-data" class="bg-white w-[95%] md:w-1/2 p-10 rounded m-auto">
            @csrf
            <h2 class="text-primary text-xl font-bold text-center mb-4">Add New Constituencies</h2>
            <div class="border-2 border-primary  rounded p-8 w-full">

                <span class="capitalize font-bold block">Constituency Name</span>
                <input type="text" name="name" class="border-2 border-primary rounded p-1 w-full mb-2">
                @error('name')
                <p class="text-xs text-black">{{$message}}</p>
                @enderror
                <span class="capitalize font-bold block">Region</span>
                <input type="text" name="region" class="border-2 border-primary rounded p-1 w-full mb-2">
                @error('region')
                <p class="text-xs text-black">{{$message}}</p>
                @enderror
                <span class="capitalize font-bold block">Woreda</span>
                <input type="text" name="woreda" class="border-2 border-primary rounded p-1 w-full mb-2">
                @error('woreda')
                <p class="text-xs text-black">{{$message}}</p>
                @enderror
        ...
        <table id="dataTable" class="w-[75%] m-auto mt-5 text-center p-5 rounded">
            <thead>
                <tr class="bg-primary text-white">
                    ...
                    <th class="py-2 px-4">Woreda</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($constituencies as $constituency )
                <tr>
                    <td class="py-2 px-4">{{$constituency->id}}</td>
                    <td class="py-2 px-4">{{$constituency->name}}</td>
                    <td class="py-2 px-4">{{$constituency->region}}</td>
                    <td class="py-2 px-4">{{$constituency->woreda}}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
        <script>
            ...
            table.addEventListener('click', function(e) {
                ...
                document.getElementById('region').value = cells[2].textContent;
                document.getElementById('woreda').value = cells[3].textContent;
            });
        </script>
        <form action="{{route('constituency')}}" id="editForm" method="post" enctype="multipart/form-data" class="bg-white w-[95%] md:w-1/2 mt-10 p-10 rounded m-auto">
            @csrf
            @method('put')
            ...
            <div class="border-2 border-primary  rounded p-8 w-full">
                ...
                @enderror
                <span class="capitalize font-bold block">Region</span>
                <input type="text" autocomplete="off" name="region" id="region" class="border-2 border-primary rounded p-1 w-full mb-2">
                @error('region')
                <p class="text-xs text-black">{{$message}}</p>
                @enderror
                <span class="capitalize font-bold block">Woreda</span>
                <input type="text" autocomplete="off" name="woreda" id="woreda" class="border-2 border-primary rounded p-1 w-full mb-2">
                @error('woreda')
                <p class="text-xs text-black">{{$message}}</p>
                @enderror
